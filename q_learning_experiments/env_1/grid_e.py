...
while epsilon <= 1:
    ...
    while not converged:
        ...
        while not over:
            ...
            if not over:
                ...
                if actions > (LEN - 1) * 4 and epsilon == 0:
                    converged = True
            elif over:
                print("over")
                avg_move.append(actions)
                if len(avg_move) >= avg_move_count:
                    if sum(avg_move[-avg_move_count:])/avg_move_count < best_action \
                            and epsilon == 0:
                        converged = True
                        print("failed to converge")
                    elif sum(avg_move[-avg_move_count:])/avg_move_count \
                            > best_action and epsilon == 0:
                        converged = True
                        print("failed to converge")
                ...
                if reward == env.target_reward:
                    ...
                    if actions < best_action:
                        ...
                        actions, win = env.play(q_table=q_table, fps=60, flip=True)
                        if win and actions <= best_action:
                            np.save(f"q_itr-{act}_eps-{starting_eps}_act-{actions}.npy", q_table)
    ...
    print("out of converged")
    ep_eps['ep'].append(act)
    ep_eps['eps'].append(starting_eps)
    # epsilon = starting_eps + 0.1
    starting_eps = epsilon
    epsilon_decay_val = (epsilon-epsilon_min) / (max_iter*0.9)
print(env.get_path(q_table))
...
plt.plot(ep_eps['eps'], ep_eps['ep'], label="episode")
plt.legend(loc=4)
plt.show()
